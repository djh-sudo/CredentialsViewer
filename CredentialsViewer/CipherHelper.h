...
public:
	...
	static std::string PBKDF2_SHA512(const void *password, int pass_len, const void *salt, int salt_len, int iteration, int szLen) {
		bool status = false;
		int szHmac = SHA512_DIGEST_LENGTH;
		char *asalt = NULL, *obuf = NULL, *dl = NULL;
		std::string key = "";
		do {
			asalt = new char[salt_len + sizeof(int)];
			if (asalt == NULL) {
				break;
			}
			memset(asalt, 0, salt_len + sizeof(int));

			obuf = new char[szHmac];
			if (obuf == NULL) {
				break;
			}
			...
			for (int i = 1; szLen > 0; ++i) {
				*(int *)(asalt + salt_len) = _byteswap_ulong(i);
				...
				for (int k = 1; k < iteration; ++k) {
					...
					for (int j = 0; j < szHmac; ++j) {
						obuf[j] ^= dl[j];
					}
					memcpy(dl, obuf, szHmac);
				}
				int r = min(szLen, szHmac);
